...
# Base per-worktree env (generated by wt-up.sh — gives POSTGRES_DB / METABASE_PORT
# for the slug), then optional .env.local credential overrides.
...
# Postgres now lives in the shared services stack (project axiome-shared).
CA=(-p axiome-shared -f docker-compose.shared.yml)
