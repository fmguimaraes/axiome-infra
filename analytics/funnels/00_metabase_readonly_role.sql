...
-- Metabase only ever needs SELECT on the analytics `analytics_events` table — nothing else in
...
-- applied its migrations (so `organization_svc.analytics_events` exists):
...
GRANT SELECT  ON          organization_svc.analytics_events TO metabase_ro;
